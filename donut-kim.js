...
function createClampBossSprite(size) {
  ...
  ict.fillStyle = '#f0f3ff';
  ict.strokeStyle = '#7b8ba5';
  ict.lineWidth = size * 0.04;

  ict.save();
  ict.translate(size * 0.12, 0);
  ict.rotate(Math.PI / 12);
  roundRect(ict, -armLength, -armWidth / 2, armLength, armWidth, armWidth / 2);
  ict.fill();
  ict.stroke();
  ict.restore();
  ...
  ict.translate(-size * 0.12, 0);
  ict.rotate(-Math.PI / 12);
  roundRect(ict, 0, -armWidth / 2, armLength, armWidth, armWidth / 2);
  ict.fill();
  ict.stroke();
  ict.restore();

  // Claw tips
  ict.fillStyle = '#c9d7f2';
  ict.beginPath();
  ict.moveTo(armLength * 0.6, -armWidth * 0.6);
  ict.lineTo(armLength * 0.85, 0);
  ict.lineTo(armLength * 0.6, armWidth * 0.6);
  ict.closePath();
  ict.fill();
  ict.stroke();
  ...
  const bodyGradient = ict.createLinearGradient(-bodyWidth / 2, -bodyHeight / 2, bodyWidth / 2, bodyHeight / 2);
  bodyGradient.addColorStop(0, '#e0f1ff');
  bodyGradient.addColorStop(1, '#a9c2e0');
  roundRect(ict, -bodyWidth / 2, -bodyHeight / 2, bodyWidth, bodyHeight, bodyWidth / 3);
  ict.fillStyle = bodyGradient;
  ict.fill();
  ict.stroke();

  // Center joint
  ict.fillStyle = '#5a6c87';
  ict.beginPath();
  ict.arc(0, 0, size * 0.12, 0, Math.PI * 2);
  ict.fill();
  ict.strokeStyle = '#223047';
  ...
  ict.fillStyle = '#ffffff';
  ict.beginPath();
  ict.arc(-eyeOffset, -size * 0.12, size * 0.08, 0, Math.PI * 2);
  ict.arc(eyeOffset, -size * 0.12, size * 0.08, 0, Math.PI * 2);
  ict.fill();
  ict.strokeStyle = '#1c2130';
  ict.lineWidth = size * 0.015;
  ict.stroke();
  ict.fillStyle = '#1c2130';
  ict.beginPath();
  ict.arc(-eyeOffset, -size * 0.12, size * 0.04, 0, Math.PI * 2);
  ict.arc(eyeOffset, -size * 0.12, size * 0.04, 0, Math.PI * 2);
  ict.fill();
  ...
  ict.lineWidth = size * 0.015;
  ict.beginPath();
  ict.arc(0, size * 0.05, size * 0.16, 0, Math.PI);
  ict.stroke();
...
const sprites = {
  ...
  boss: createClampBossSprite(ENEMY_SIZE * 2.4),
  blade: createRoundedSprite(BLADE_SIZE, '#9ad8ff', '#3370c2'),
...
function recomputeBlades(dt) {
  ...
  if (bladeCount > 0) {
    ...
    for (let i = 0; i < bladeCount; i++) {
      ...
      state.bladePositions.push(vectorAdd(state.playerPos, offset));
    }
  }
}
...
function handleEnemies(dt) {
  ...
  for (const enemy of state.enemies) {
    ...
    let killed = false;
    if (state.bladePositions.length > 0) {
      for (const bladePos of state.bladePositions) {
        if (circleIntersects(enemy.pos, ENEMY_SIZE / 2, bladePos, BLADE_SIZE / 2)) {
          grantKillReward();
          onEnemyRemoved(enemy);
          killed = true;
          break;
  ...
  if (state.boss) {
    if (state.playerInvuln <= 0 && circleIntersects(state.boss.pos, ENEMY_SIZE, state.playerPos, PLAYER_SIZE / 2)) {
    ...
    if (!state.gameOver && state.bladePositions.length > 0 && state.boss) {
      const id = 'boss';
      if (!state.bladeCooldowns.has(id)) {
        for (const bladePos of state.bladePositions) {
          if (circleIntersects(state.boss.pos, ENEMY_SIZE, bladePos, BLADE_SIZE / 2)) {
            state.boss.health -= 1;
            state.score += BOSS_HIT_SCORE;
...
function render() {
  ...
  for (const bladePos of state.bladePositions) {
    drawSprite(sprites.blade, bladePos, BLADE_SIZE);
  }
