...
class DonutRankingDB {
  ...
  async saveRanking(nickname, character, stage, survivalTime, finalScore, gameType = 'normal') {
    try {
...
export async function savePlayerRanking(nickname, character, stage, survivalTime, finalScore) {
  return await rankingDB.saveRanking(nickname, character, stage, survivalTime, finalScore, 'normal');
}
